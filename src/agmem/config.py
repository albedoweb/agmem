...
Before any non-trivial task in this repo, run **BOTH** commands **before your first
Edit or Write**:

```bash
agmem hot                                                # instant rules cache (~500 tokens)
agmem context "<short task description>" -n 8 --session  # task-specific retrieval — DO NOT skip
```

`hot` alone gives only the project's hard rules. `context` retrieves the task-specific
facts and patterns — that's where the real lift comes from. Running `hot` without
`context` is the most common discovery failure; don't make it.

`--session` makes follow-up queries within the same session avoid returning chunks you
already saw and surface sibling sections instead, plus a "Haven't seen yet" hint at the
end. Drop `--session` for one-shot CI usage.
